...
_PID_FILE = Path.home() / ".crispcode" / "crisp-core.pid"


# 尝试连接 daemon，成功则正常返回，失败则抛出 ConnectionRefusedError/OSError
async def _ping_check(config: CrispConfig) -> None:
...
def cmd_core_status(config: CrispConfig) -> None:
    try:
...
def cmd_core_start(config: CrispConfig) -> None:
    try:
        asyncio.run(_ping_check(config))
...
def cmd_core_stop(config: CrispConfig) -> None:
    pid = _running_pid()
    if pid is None:
        print("not running")
        return
