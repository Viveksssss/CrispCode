...
def _process_line(
    ...
    *,
    ...
    if runs_id is not None and record.runs_id != runs_id:
        return
...
def _print_record(record: TraceRecord) -> None:
    ...
    parts: list[str] = []
    if record.runs_id:
        parts.append(f"run={record.runs_id[:8]}")
...
def _summarize(record: TraceRecord) -> str:
    ...
    if kind == "response":
        ...
        if isinstance(result, dict) and "runs_id" in result:
            return f"runs_id={result['runs_id'][:8]}"
