...
class PermissionManager:
    ...
    def __init__(
        ...
        *,
        ...
        self._polices: dict[str, ToolPolicy] = policies or dict(DEFAULT_POLICIES)
    ...
    def evaluate(self, tool_name: str, params: dict[str, Any]) -> PermissionDecision:
        ...
        policy = self._polices.get(tool_name)
    ...
    def respond(self, tool_use_id: str, decision: str) -> None:
        ...
        if req is None:
            logger.warning("permission.respond: unknow tool_use_id=%s", tool_use_id)
            return
        ...
        if decision == "always_allow":
            ...
            if self._policy_file is not None:
                try:
                    ...
                    logger.info(
                        "permission: policy.toml written path=%s", self._policy_file
                    )
                except Exception:
                    logger.exception(
    ...
    def cancel_session(
        self, session_id: str, reason: str = "client_disconnected"
    ) -> None:
        """客户端断连时拒绝该 session 所有待审批请求，防止 Future 永久挂起"""
        to_cancel = [
            uid for uid, req in self._pending.items() if req.session_id == session_id
        ]
