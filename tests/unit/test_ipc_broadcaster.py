...
def _run_started(runs_id: str = "r1") -> RunStartedEvent:
    return RunStartedEvent(runs_id=runs_id, goal="test", ts="2026-01-01T00:00:00Z")
...
async def test_subscriber_receives_matching_event() -> None:
    ...
    writer.write.call_count == 2  # type: ignore[attr-defined]
...
async def test_topic_glob_matches_step_not_run() -> None:
    ...
    step_event = StepStartedEvent(runs_id="r1", step=1, ts="2026-01-01T00:00:00Z")
...
# 功能：验证 scope="global" 的订阅能收到任意 runs_id 的事件
# 设计：发布两个不同 runs_id 的事件，断言两次都写入，确认 global scope 不过滤 runs_id 字段
async def test_scope_global_receives_all_runs_ids() -> None:
    broadcaster = IpcEventBroadcaster()
    writer = _make_writer()
...
# 功能：验证 scope="run:<id>" 只接收匹配 runs_id 的事件，过滤其他 runs_id
# 设计：订阅 scope="run:abc"，发布 runs_id="abc" 和 runs_id="xyz"，断言只写入一次，验证 run-specific scope 的过滤语义
async def test_scope_run_specific_filters_other_runs_ids() -> None:
    broadcaster = IpcEventBroadcaster()
    writer = _make_writer()
