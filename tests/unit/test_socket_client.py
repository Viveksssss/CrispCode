...
async def test_event_push_routed_to_handler() -> None:
    ...
    async def handle(
        ...
        writer.write(json.dumps(resp).encode() + b"\n")
        push = {"kind": "event", "event": {"type": "run.started", "runs_id": "r1"}}
...
async def test_run() -> None:
    async def prints(event: dict[str, Any]) -> None:
    ...
    async def handle(
        ...
        resp = {
            "jsonrpc": "2.0",
            "id": req["id"],
            "resutest_tui_app.pylt": {"pong": True},
        }
    ...
    async with server:
        client = SocketClient("127.0.0.1", port)
        client2 = SocketClient("127.0.0.1", port)
        await client.connect()
        await client2.connect()
        client.on_event(prints)
        client2.on_event(prints)
        loop_task = asyncio.create_task(client.run_event_loop())
        loop_task2 = asyncio.create_task(client2.run_event_loop())
        try:
            await asyncio.wait_for(
                client.send_command("event.subscribe", {"topics": ["run.*"]}),
                timeout=2.0,
        ...
        try:
            await asyncio.wait_for(
                client2.send_command("event.subscribe", {"topics": ["run.*"]}),
                timeout=2.0,
            )
        except Exception as e:
            print(e)
