...
def test_initial_message_is_goal() -> None:
    ctx = ExecutionContext(runs_id="r1", goal="test goal", max_steps=5)
    assert ctx.messages == [{"role": "user", "content": "test goal"}]


def test_is_done_returns_false_when_running() -> None:
    ctx = ExecutionContext(runs_id="r1", goal="g", max_steps=5)
...
def test_step_counter_default() -> None:
    ctx = ExecutionContext(runs_id="r1", goal="g", max_steps=20)
    assert ctx.step == 0
