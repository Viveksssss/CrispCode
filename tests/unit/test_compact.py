...
from crispcode.core.llm.types import LlmResponse, UsageState
...
def _stub_provider(
    ...
    provider.chat = AsyncMock(
        return_value=LlmResponse(
            ...
            text=summary,
            usage=UsageState(input_token=100, output_token=30),
        )
    )
    return provider
...
def test_compact_replaces_context_messages(tmp_path: Path) -> None:
    ...
    bus = EventBus()
    compactor = Compactor(bus, tmp_path, "sess-1")
    ctx = ExecutionContext(runs_id="r1", goal="test", max_steps=5)
...
def test_compact_failure_preserves_context(tmp_path: Path) -> None:
    ...
    result = asyncio.run(
        compactor.compact(ctx, provider)
    )
