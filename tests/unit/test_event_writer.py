...
async def test_event_writer_writes_jsonl(tmp_path: Path) -> None:
    ...
    event = RunStartedEvent(
        runs_id="run-1", goal="test goal", ts="2026-05-11T00:00:00Z"
    )
...
async def test_event_writer_appends_multiple_events(tmp_path: Path) -> None:
    ...
    async with EventWriter(path) as writer:
        await writer.handle(
            RunStartedEvent(runs_id="r1", goal="g1", ts="2026-05-11T00:00:00Z")
        )
        finished = RunFinishedEvent(
            runs_id="r1", status="success", steps=2, ts="2026-05-11T00:00:01Z"
        )
...
async def test_event_writer_subscribe_via_bus(tmp_path: Path) -> None:
    path = tmp_path / "events.jsonl"
    bus = EventBus()
    event = RunStartedEvent(runs_id="r1", goal="g", ts="2026-05-11T00:00:00Z")
    ...
    lines = path.read_text().strip().splitlines()
    assert len(lines) == 1
    assert json.loads(lines[0])["runs_id"] == "r1"
...
async def test_event_writer_oserror_is_logged(
    ...
    with caplog.at_level(logging.ERROR, logger="crispcode.core.events.writer"):
        async with EventWriter(path) as writer:
