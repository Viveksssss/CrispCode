...
import crispcode.core.tools.invocation as inv_mod
...
async def _run(tool: BaseTool, *, monkeypatch: pytest.MonkeyPatch) -> tuple[ToolResult, list]:
    ...
    result = await invoke_tool(registry, _call(tool.name), bus, runs_id="r")
...
async def test_timeout_no_retry(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    bus.subscribe(_collect)
    result = await invoke_tool(registry, _call("slow"), bus, runs_id="r", timeout=0.05)
