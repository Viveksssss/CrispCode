...
from crispcode.core.transport.socket_client import SocketClient


# 功能：验证 agent.run 命令返回非空 runs_id，且 daemon 随即广播 run.started 事件
# 设计：用 SocketClient 封装 IPC 层，asyncio.Event 等待事件而非轮询，
#       timeout=5s 防测试挂起；run.started 在 LLM 调用前触发，无需真实 API Key
async def test_agent_run_returns_runs_id_and_emits_started(
    ...
    try:
        await client.send_command(
            "event.subscribe", {"topics": ["run.*"], "scope": "global"}
        )
        result = await client.send_command("agent.run", {"goal": "hello"})

        assert result.get("runs_id"), "runs_id must be non-empty"
        returned_runs_id: str = result["runs_id"]

        await asyncio.wait_for(started_event.wait(), timeout=5.0)
        assert received.get("runs_id") == returned_runs_id
...
#       稍作等待后断开；client2 用 replay_from_run=runs_id 订阅，断言 replayed_count > 0，
...
async def test_disconnect_and_replay_from_run(
    ...
    started_event: asyncio.Event = asyncio.Event()
    runs_id_holder: list[str] = []

    async def on_event(event: dict[str, Any]) -> None:
        if event.get("type") == "run.started":
            runs_id_holder.append(event.get("runs_id", ""))
            started_event.set()
    ...
    assert runs_id_holder, "run.started was never received"
    runs_id = runs_id_holder[0]
    ...
    try:
        result = await client2.send_command(
            "event.subscribe",
            {
                "topics": ["run.*"],
                "scope": "global",
                "replayed_from_run": runs_id,
            },
        )
        assert (
            result.get("replayed_count", 0) > 0
        ), f"Expected replayed_count > 0 for runs_id={runs_id!r}, got {result}"
    finally:
        loop2.cancel()
