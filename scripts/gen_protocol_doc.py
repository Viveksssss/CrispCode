#!/usr/bin/env python3
"""Generate WIRE_PROTOCOL.md from pydantic models in crispcode.core.bus."""
...
def generate() -> str:
    ...
    sections = [
        ...
        "## Transport\n\n",
        "- TCP loopback `127.0.0.1:7437` (override via `CRISP_HOST` / `CRISP_PORT`)\n",
        ...
        "\n",
        _model_section(
            "EventSubscribeCommand", EventSubscribeCommand, subscribe_req_example
        ),
        "\n",
        _model_section(
            "EventSubscribeResult", EventSubscribeResult, subscribe_resp_example
        ),
        "\n",
        _model_section(
            "SessionCreateCommand", SessionCreateCommand, session_create_req_example
        ),
        "\n",
        _model_section(
            "SessionCreateResult", SessionCreateResult, session_create_resp_example
        ),
        "\n",
        _model_section(
            "SessionSendMessageCommand",
...
def main() -> None:
    ...
    parser.add_argument(
        "--check", action="store_true", help="Verify file matches generated output"
    )
    ...
    if args.check:
        ...
        if output_path.read_text() != content:
            print(
                f"ERROR: {output_path} out of sync with code — run: make docs",
                file=sys.stderr,
            )
            sys.exit(1)
